	...
	for (let i = 0; i < gridSize; i++) {
		for (let j = 0; j < gridSize; j++) {
			...
			tempchild.setAttribute(
				"Style",
				`border:solid 1px white; background-color:white; height:${cellSize}px; width:${cellSize}px; `
			);
			tempchild.addEventListener("mousedown", (event) => {
				tempchild.style.backgroundColor = "green";
			});
			tempchild.addEventListener("mouseover", async (event) => {
				if (draw == true) {
					tempchild.style.backgroundColor = "green";
				}
			});
